...
def run(
        source='0',
        yolo_weights=ROOT / 'yolov8n.pt',  # model.pt path(s),
    ...
    if webcam:
        ...
        # depth_frame = dataset.depth_image
        bs = len(dataset)
    else:
        dataset = LoadImages(
            source,
                ...
                if len(outputs[i]) > 0:
                    ...
                    for j, (output) in enumerate(outputs[i]):
                        ...
                        if save_vid or save_crop or show_vid:  # Add bbox/seg to image
                            ...
                            label = None if hide_labels else (f'{id} {names[c]} z:{coor_z} mm' if hide_conf else \
                                (f'{id} {conf:.2f} z:{coor_z} mm' if hide_class else f'{id} {names[c]} {conf:.2f} z:{coor_z} mm'))
                            color = colors(c, True)
