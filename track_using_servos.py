                ...
                if len(outputs[i]) > 0:
                    ...
                    for j, (output) in enumerate(outputs[i]):
                        ...
                        if(names[int(cls)] == "cat" and id == 1):
                            ...
                            #print(f"bbox_left:{bbox_left}, bbox_top:{bbox_top}, bbox_w:{bbox_w}, bbox_h:{bbox_h}")
                            print(f"center coordinate {c_x}, {c_y}")

                            error_y  = 480/2 - c_y
                            integral_y_error += (error_y + last_error_y)*time_interval/2
                            angle_y = (Kp_y*error_y  + 
                                       Ki_y*integral_y_error +
                                       Kd_y*(error_y - last_error_y)/time_interval)
        
                            error_x = 640/2 - c_x
                            integral_x_error += (error_x + last_error_x)*time_interval/2
                            angle_x = (Kp_p*error_x  + 
                                       Ki_p*integral_x_error +
                                       Kd_p*(error_x - last_error_x)/time_interval)  

                            print(f"error_y : {error_y}, error_x : {error_x}")
                            GoalAngles[0] = GoalAngles[0] + angle_y
                            GoalAngles[1] = GoalAngles[1] + angle_x
